...
def super_ham(part_no, part_states, d, truncate):
    ...
    print(len(single_part_configs),"single_part_configs")
    ...
    fermi_states=[level[1] for level in sp_0.levels if level[0]]
    not_fermi_states=[level[1] for level in sp_0.levels if not level[0]]
    ...
    def basic_ham(c1, c2): # not using state information
        match, h=0, 0
        ...
        # if truncate>=0:
        #     ord_1, ord_2 = count_excitation(c1), count_excitation(c2)
        #     if not (ord_1 > truncate or ord_2 > truncate):
        #         h-=2.0*g*float(match/4.0)
        # else:
        #     h-=2.0*g*float(match/4.0)
        # if matches
    ...
    for row in range(no_pairs_configs):
        for col in range(no_pairs_configs):
            ...
            # print(basic_ham(c1, c2))
            Ham[row,col]=basic_ham(c1, c2)
            # Ham[row,col] = full_ham(row, col)
...
for no_particles, no_states, truncate in [[8, 8*2, -1], [8, 10*2, -1]]:#, [8, 12*2, -1]
    ...
    Ham = super_ham(no_particles, no_states, 1, truncate)
